...
int main(void)
{
	...
	*/
	...
	if(UART_Config(9600))
	{
		LCD_STRING_ON(0, LCD_LINE_2, (uint8_t *)"串口配置失败！      ", LCD_COLOR_RED);
		Error_Handler();
	}
	LCD_STRING_ON(0, LCD_LINE_2, (uint8_t *)"发送内容：", LCD_COLOR_WHITE);
	LCD_Showphrase(0, LCD_LINE_3, Tx_Buffer);
	LCD_STRING_ON(0, LCD_LINE_7, (uint8_t *)"每1s发送一次！", LCD_COLOR_WHITE);
