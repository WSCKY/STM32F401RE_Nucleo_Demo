...
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
__IO uint32_t VS1003Timeout = VS1003_TIMEOUT;
...
uint8_t VS1003_Send_MusicData(uint8_t *DataBuffer)
{
	if(VS1003_DREQ_STATUS())
	{
		...
		HAL_SPI_Transmit_DMA(&SpiHandle, (uint8_t *)DataBuffer, 32);
...
uint8_t VS1003_Sine_Test(uint8_t TestValue, uint32_t TestTime)
{
	...
	HAL_SPI_Transmit_DMA(&SpiHandle, (uint8_t *)StartSineTest, 8);
	/* Wait for the end of the transfer */
	while(HAL_SPI_GetState(&SpiHandle) != HAL_SPI_STATE_READY);
	HAL_Delay(TestTime);
	VS1003_XDCS_HIGH();
	VS1003_XDCS_LOW();
	HAL_SPI_Transmit_DMA(&SpiHandle, (uint8_t *)StopSineTest, 8);
	/* Wait for the end of the transfer */
	while(HAL_SPI_GetState(&SpiHandle) != HAL_SPI_STATE_READY);
	HAL_Delay(20);
	VS1003_XDCS_HIGH();
...
uint8_t VS1003_RAM_Test(void)
{
	...
	VS1003_XDCS_LOW();
	HAL_SPI_Transmit_DMA(&SpiHandle, (uint8_t *)RAMTestBuff, 8);
	/* Wait for the end of the transfer */
	while(HAL_SPI_GetState(&SpiHandle) != HAL_SPI_STATE_READY);
	HAL_Delay(50);
	VS1003_XDCS_HIGH();
...
//	VS1003_SendByte(DUMMY_BYTE);
//	VS1003Timeout = VS1003_TIMEOUT;
//	SPI_ReadDeviceRegister(SCI_MODE, &regit);
//	while(regit != 0x0800)
//	{
//		if((VS1003Timeout --) == 0) return VS1003_ERROR_UserCallback();
		SPI_WriteDeviceRegister(SCI_MODE, 0x0804);
		HAL_Delay(2);
//		SPI_ReadDeviceRegister(SCI_MODE, &regit);
//	}
	VS1003Timeout = VS1003_TIMEOUT;
...
//	VS1003Timeout = VS1003_TIMEOUT;
//	SPI_ReadDeviceRegister(SCI_CLOCKF, &regit);
//	while(regit != 0x9800)
//	{
//		if((VS1003Timeout --) == 0) return VS1003_ERROR_UserCallback();
...
//	HAL_GPIO_WritePin(VS1003_RESET_GPIO_PORT, VS1003_RESET_PIN, GPIO_PIN_SET);
...
//		HAL_GPIO_WritePin(VS1003_DREQ_GPIO_PORT, VS1003_DREQ_PIN, GPIO_PIN_SET);
	}
	else
	{
...
static uint8_t VS1003_SendByte(uint8_t byte)
{
	uint8_t ReceByte, TranByte;
	TranByte = byte;
	if(HAL_SPI_TransmitReceive_DMA(&SpiHandle, &TranByte, &ReceByte, 1) != HAL_OK)
		return VS1003_ERROR_UserCallback();
	while(HAL_SPI_GetState(&SpiHandle) != HAL_SPI_STATE_READY);
	return ReceByte;
}

#ifdef USE_DEFAULT_ERROR_CALLBACK
