...
uint8_t gif_dispimage(FIL *gfile,gif89a* gif,uint16_t x0,uint16_t y0,int Transparency, uint8_t Disposal) 
{
	...
	f_read(gfile, &lzwlen, 1, (UINT*)&readed);/* 获取LZW长度 */ 
...
void gif_clear2bkcolor(uint16_t x,uint16_t y,gif89a* gif,ImageScreenDescriptor pimge)
{
	...
	if(gif->gifISD.yoff>pimge.yoff)
	{
		...
		if(x0<x1&&y0<y1&&x1<320&&y1<320)	 /* 设定xy,的范围不能太大 */
			pic_phy.fill(x0,y0,x1,y1,color); /* 填充矩形色块★ */  
	}
...
{		  
		...
		{		 
			case GIF_INTRO_IMAGE://图像描述
 				...
 				while(1)
				{
					...
					if(temp==0)break;
					readed=f_tell(gfile);/* 还存在下一块 */
					if(f_lseek(gfile,readed+temp))break;//继续向后偏移	 
			    }
				if(temp!=0)return 1;/* 出错 */
				return 0;
			case GIF_INTRO_TERMINATOR:/* 解码完成 */
				return 2;
			case GIF_INTRO_EXTENSION:
				/* Read image extension 读取图像扩展块消息*/
	...
	}while(Introducer != GIF_INTRO_TERMINATOR);/* 读取到结束符 */
	return 0;
}
...
/** 
  * @brief  解码一个gif文件(本例子不能显示尺寸大于给定尺寸的gif图片!)
