...
uint8_t stdbmp_decode(const uint8_t *filename) 
{
	...
	uint16_t  realx = 0;/* x,y的实际坐标 */
	uint16_t realy = 0;
	uint8_t  yok = 1;  				   
	uint8_t res;
	uint8_t *databuf;    		//数据读取存放地址
 	uint16_t readlen = 1*1024;		//一次从SD卡读取的字节数长度
	...
	databuf = bmpreadbuf;
	f_bmp = &f_bfile;
	res = f_open(f_bmp, (const XCHAR*)filename, FA_READ);/* 打开文件 */
	if(res == 0)/* 文件打开成功 */
	{ 
		f_read(f_bmp, databuf, readlen, (UINT*)&byre);	//读出readlen个字节  
...
uint8_t minibmp_decode(uint8_t *filename,uint16_t x,uint16_t y,uint16_t width,uint16_t height,uint16_t acolor,uint8_t mode)//尺寸小于240*320的bmp图片解码.
{
	...
	uint16_t i,j;
	uint8_t *databuf;    		//数据读取存放地址
 	uint16_t readlen = 1024;	    //一次从SD卡读取的字节数长度,不能小于240*3!!!
